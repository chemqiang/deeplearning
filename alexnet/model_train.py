...
from model import aLexNet#导入模型(重要)


def train_val_data_progress():#训练集和验证集的划分
    train_data = FashionMNIST(root="./data",    #数据集的存储路径
                            train=True,     #指定加载训练集
                            transform=transforms.Compose([transforms.Resize((227,227)), transforms.ToTensor()]),      #对图像进行预处理，先将图像调整为224x224的大小，然后将其转换为张量    
                            download=True)      #下载数据集
