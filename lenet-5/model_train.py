...
from model import leNet#导入模型(重要)


def train_val_data_progress():#训练集和验证集的划分
    train_data = FashionMNIST(root="./data",    #数据集的存储路径
                            train=True,     #指定加载训练集
                            transform=transforms.Compose([transforms.Resize((28,28)), transforms.ToTensor()]),      #对图像进行预处理，先将图像调整为224x224的大小，然后将其转换为张量    
                            download=True)      #下载数据集
    ...
    train_data_loader = torch.utils.data.DataLoader(train_data,      #加载训练集
                                                ...
                                                num_workers=2,      #加载数据的线程数，0表示不使用多线程
                                                shuffle=True)       #数据是否打乱
...
if __name__ == "__main__":
    ...
    train_progress = train_model_progress(LeNet, train_data_loader, val_data_loader, num_epochs=20)      #训练模型并获取训练进度
